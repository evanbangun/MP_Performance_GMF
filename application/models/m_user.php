...
{
	public function tampilassignment($id_user, $ac_type, $resp)
	{
		...
		// 							LIMIT 100");
		$query = $this->db->query("	SELECT tmp.ms_num, tmp.ac_type, tmp.task_code, tmp.rvcd, tmp.resp, tmp.id_user,  tmp.status, tmp.descr, tmp.camp_sg, tmp.intval, tmp.ref, tmp.eva_ver FROM (SELECT etp.ms_num as ms_num, etp.ac_type as ac_type, md.task_code as task_code, md.rvcd as rvcd, md.resp as resp,
